...
$Global:VCFStretchVersion = '2.0.5'
...
function Get-VCenterClusterNetworkFallback {
    ...
    # Workload/VI clusters can live on a different vCenter than the management cluster.
    # If the selected SDDC cluster reports a different vCenter than the UI field, switch
    # the PowerCLI connection to the cluster's registered vCenter using the UI credentials.
    ...
    $vcServer = $script:VCenterServer
    $serverParams = @{}
    if ($vcServer) { $serverParams['Server'] = $vcServer }

    $rows = @()
    $cluster = $null
    try {
        $cluster = Get-Cluster @serverParams -Name $clusterName -ErrorAction Stop
    } catch {
        Write-Log "vCenter cluster lookup by name '$clusterName' failed: $($_.Exception.Message)" 'WARN'
    }
    ...
    if ($rows.Count -eq 0 -and $sddcClusterMor) {
        ...
        foreach ($vid in ($viewIds | Select-Object -Unique)) {
            try {
                ...
                Write-Log "vCenter cluster lookup by MoRef '$vid' failed: $($_.Exception.Message)" 'WARN'
            }
        }
    }
...
function Ensure-DefaultNetworkMappings {
    ...
    if ($det.VdsCount -gt 1) { Write-Log "Multiple vDS detected; using NSX/overlay vDS '$($det.PrimaryVdsName)' for the network profile and ignoring the other vDS objects for NSX uplinks." 'INFO' }
...
function Convert-DetectedVmnicMapToText { param($DetectedNetwork) $v1=$DetectedNetwork.PrimaryVdsName; if(-not $v1){$v1=$DetectedNetwork.VdsNames[0]}; return "vmnic0->$v1/uplink1; vmnic1->$v1/uplink2" }
function New-VmnicMapForNsxVds { param([string]$VdsName) return "vmnic0->$VdsName/uplink1; vmnic1->$VdsName/uplink2" }
...
function Get-SelectedNsxVdsName {
    ...
    if ($script:chkOverrideNsxVds -and $script:chkOverrideNsxVds.IsChecked) {
        ...
        Write-Log "NSX vDS override enabled; using '$manual' for network profile and AZ2 host vmnic mapping." 'INFO'
        return $manual
    }
