...
$Global:VCFStretchVersion = '2.0.3'
...
function Install-RequiredModule {
    ...
    try {
        Write-Log "Installing prerequisite module $FriendlyName ($Name) for CurrentUser..." 'INFO'
        ...
        try { Import-Module $Name -ErrorAction SilentlyContinue | Out-Null } catch {}
        Write-Log "Installed prerequisite module $FriendlyName." 'INFO'
        Prereq-Check | Out-Null
    } catch {
        Write-Log "Install $FriendlyName failed: $($_.Exception.Message)" 'ERROR'
        [System.Windows.MessageBox]::Show($_.Exception.Message,"Install $FriendlyName failed",'OK','Error') | Out-Null
    }
}
...
function Get-VCenterClusterNetworkFallback {
    ...
    if ($sddcVcFqdn -and $uiVcFqdn -and ($sddcVcFqdn -ine $uiVcFqdn)) {
        ...
        try {
            ...
            try { $script:lblVCenterStatus.Text='Verified'; $script:lblVCenterStatus.Foreground=[Windows.Media.Brushes]::LightGreen } catch {}
