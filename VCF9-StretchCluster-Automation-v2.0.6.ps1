...
.DESCRIPTION
  ...
  Includes:
    ...
    - v2.0.6: background prerequisite installs to keep UI responsive, aligned common depot dark theme, removed duplicate large header, Ready/Running indicator, simplified popups, Open Output fix, and Witness DNS/vCenter validation
...
$Global:VCFStretchVersion = '2.0.6'
...
function Set-StatusText {
    ...
    if (-not $Label) { return }
    $Label.Text = $Text
    if ($State -eq 'OK') { $Label.Foreground = [Windows.Media.BrushConverter]::new().ConvertFromString('#7FD37F') }
    elseif ($State -eq 'FAIL') { $Label.Foreground = [Windows.Media.BrushConverter]::new().ConvertFromString('#D97878') }
    elseif ($State -eq 'WARN') { $Label.Foreground = [Windows.Media.BrushConverter]::new().ConvertFromString('#D6C15A') }
    else { $Label.Foreground = [Windows.Media.BrushConverter]::new().ConvertFromString('#E6E6E6') }
}


function Set-RunStatus {
    param([string]$Text='Ready',[string]$State='Ready')
    try {
        if ($script:lblRunStatus) {
            $script:lblRunStatus.Text = $Text
            if ($State -eq 'Running') { $script:lblRunStatus.Foreground = [Windows.Media.BrushConverter]::new().ConvertFromString('#6FA8DC') }
            elseif ($State -eq 'Failed') { $script:lblRunStatus.Foreground = [Windows.Media.BrushConverter]::new().ConvertFromString('#D97878') }
            else { $script:lblRunStatus.Foreground = [Windows.Media.BrushConverter]::new().ConvertFromString('#7FD37F') }
        }
    } catch { }
}

function Start-UiHeartbeat {
    if ($script:HeartbeatTimer) { try { $script:HeartbeatTimer.Stop() } catch { } }
    $script:HeartbeatOn = $false
    ...
    $script:HeartbeatTimer.Add_Tick({
        if ($script:IsBusy) {
            ...
            if ($script:HeartbeatOn) { Set-RunStatus -Text 'Running' -State 'Running' }
            else { Set-RunStatus -Text 'Running .' -State 'Running' }
        }
    })
...
function Set-Busy {
    ...
    $script:IsBusy = $Busy
    try {
        $controls = @($script:btnRecheck,$script:btnInstallImportExcel,$script:btnInstallPCLI,$script:btnInstallVCFPCLI,$script:btnConnect,$script:btnVerifyVCenter,$script:btnConnectNsx,$script:btnDetectNetwork,$script:btnPickHosts,$script:btnGenerate,$script:btnValidate,$script:btnExecute,$script:btnLoadConfig,$script:btnSaveConfig,$script:btnBrowseReports,$script:btnOpenOut)
        foreach ($c in $controls) { if ($c) { $c.IsEnabled = -not $Busy } }
        if ($Busy) { Set-RunStatus -Text 'Running' -State 'Running'; Start-UiHeartbeat }
        else { Stop-UiHeartbeat; Set-RunStatus -Text 'Ready' -State 'Ready'; Refresh-TopologyUI }
    } catch { }
}
...
function Start-BackgroundTask {
    param(
        ...
        [string]$SuccessTitle = 'Task'
    )
    if ($script:CurrentJob) { throw 'A task is already running.' }
    Write-Log "Starting background task: $TaskName" 'INFO'
    ...
    $script:JobTimer.Add_Tick({
        try {
            $out = Receive-Job -Job $script:CurrentJob -Keep:$false -ErrorAction SilentlyContinue
            foreach ($line in @($out)) { if ($null -ne $line) { Write-Log ([string]$line) 'INFO' } }
            if ($script:CurrentJob.State -in @('Completed','Failed','Stopped')) {
                $state = $script:CurrentJob.State
                $errs = @($script:CurrentJob.ChildJobs | ForEach-Object { $_.JobStateInfo.Reason } | Where-Object { $_ })
                try { $script:JobTimer.Stop() } catch { }
                Remove-Job -Job $script:CurrentJob -Force -ErrorAction SilentlyContinue
                $script:CurrentJob = $null
                if ($state -eq 'Completed') {
                    Write-Log "$TaskName completed." 'INFO'
                    if ($TaskName -match '^Install prerequisite') { try { Prereq-Check | Out-Null } catch { Write-Log "Prerequisite recheck after install failed: $($_.Exception.Message)" 'WARN' } }
                    Show-InfoMessage -Message $SuccessMessage -Title $SuccessTitle
                    Set-Busy $false
                } else {
                    $detail = if ($errs.Count -gt 0) { ($errs | ForEach-Object { $_.Exception.Message }) -join ' ' } else { "$TaskName failed with job state $state." }
                    Write-Log $detail 'ERROR'
                    Set-RunStatus -Text 'Failed' -State 'Failed'
                    Show-ErrorMessage -Message "$TaskName failed. Check the log for details." -Title $SuccessTitle
                    Set-Busy $false
                }
            }
        } catch {
            Write-Log "Background task monitor failed: $($_.Exception.Message)" 'ERROR'
            Set-RunStatus -Text 'Failed' -State 'Failed'
            Set-Busy $false
        }
    })
...
function Show-InfoMessage { param([string]$Message,[string]$Title='VCF Stretch Cluster Automation') [System.Windows.MessageBox]::Show($Message,$Title,'OK','Information') | Out-Null }
function Show-ErrorMessage { param([string]$Message,[string]$Title='VCF Stretch Cluster Automation') [System.Windows.MessageBox]::Show($Message,$Title,'OK','Error') | Out-Null }
...
function Install-RequiredModule {
    ...
    try {
        ...
        Set-Busy $true
        Write-Log "Installing prerequisite module $FriendlyName ($Name) in background..." 'INFO'
        $accept = [bool]$AcceptLicense
        Start-BackgroundTask -TaskName "Install prerequisite module $FriendlyName" -SuccessTitle "Install $FriendlyName" -SuccessMessage "Installed prerequisite module $FriendlyName." -ArgumentList @($Name,$FriendlyName,$accept) -ScriptBlock {
            param($Name,$FriendlyName,$AcceptLicense)
            $ErrorActionPreference = 'Stop'
            $ProgressPreference = 'SilentlyContinue'
            Write-Output "Installing prerequisite module $FriendlyName ($Name) for CurrentUser..."
            try {
                if (-not (Get-PackageProvider -Name NuGet -ErrorAction SilentlyContinue)) {
                    Write-Output 'Installing NuGet package provider if required...'
                    Install-PackageProvider -Name NuGet -MinimumVersion 2.8.5.201 -Force -Scope CurrentUser | Out-Null
                }
            } catch { Write-Output "NuGet provider check warning: $($_.Exception.Message)" }
            try { Set-PSRepository -Name PSGallery -InstallationPolicy Trusted -ErrorAction SilentlyContinue } catch { }
            $params = @{ Name=$Name; Scope='CurrentUser'; Force=$true; AllowClobber=$true; SkipPublisherCheck=$true; ErrorAction='Stop' }
            if ($AcceptLicense) { $params['AcceptLicense'] = $true }
            Install-Module @params
            Write-Output "Installed prerequisite module $FriendlyName."
        }
    } catch {
        Write-Log "Install $FriendlyName failed: $($_.Exception.Message)" 'ERROR'
        Set-RunStatus -Text 'Failed' -State 'Failed'
        Show-ErrorMessage -Message "Install $FriendlyName failed. Check the log for details." -Title "Install $FriendlyName"
        Set-Busy $false
    }
}
...
function Get-VCenterClusterNetworkFallback {
    ...
    $sddcCluster = $null
    try {
        if ($script:SddcSession -and $cid) { $sddcCluster = Invoke-SddcApi -Session $script:SddcSession -Method GET -Path ("/v1/clusters/{0}" -f $cid) }
    } catch { Write-Log "Could not read SDDC cluster detail for vCenter fallback: $($_.Exception.Message)" 'WARN' }

    $sddcVcFqdn = ''
    $sddcClusterMor = ''
    try { $sddcVcFqdn = ([string]$sddcCluster.vcenter.fqdn).Trim() } catch {}
    try { $sddcClusterMor = ([string]$sddcCluster.vcenter.managedObjectReferenceId).Trim() } catch {}

    $uiVcFqdn = ''
    try { $uiVcFqdn = ([string]$script:txtVCenterFqdn.Text).Trim() } catch {}
    if ($sddcVcFqdn -and $uiVcFqdn -and ($sddcVcFqdn -ine $uiVcFqdn)) {
        Write-Log "Selected SDDC cluster is registered to vCenter '$sddcVcFqdn', but UI is connected to '$uiVcFqdn'. Attempting to switch vCenter context for detection." 'WARN'
        try {
            Import-Module VMware.VimAutomation.Core -ErrorAction SilentlyContinue | Out-Null
            Set-PowerCLIConfiguration -InvalidCertificateAction Ignore -Confirm:$false | Out-Null
            $vcUser = ([string]$script:txtVCenterUser.Text).Trim()
            $vcPass = [string]$script:pbVCenterPass.Password
            if ([string]::IsNullOrWhiteSpace($vcUser) -or [string]::IsNullOrWhiteSpace($vcPass)) { throw 'vCenter user/password fields are required to auto-switch vCenter context.' }
            $cred = [pscredential]::new($vcUser,(ConvertTo-SecureString $vcPass -AsPlainText -Force))
            $script:VCenterServer = Connect-VIServer -Server $sddcVcFqdn -Credential $cred -WarningAction SilentlyContinue -ErrorAction Stop
            $script:VCenterVerified = $true
            try { $script:txtVCenterFqdn.Text = $sddcVcFqdn } catch {}
            try { $script:lblVCenterStatus.Text='Verified'; $script:lblVCenterStatus.Foreground=[Windows.Media.BrushConverter]::new().ConvertFromString('#7FD37F') } catch {}
            Write-Log "Switched vCenter context to cluster registered vCenter: $sddcVcFqdn" 'INFO'
        } catch {
            Write-Log "Automatic vCenter switch to '$sddcVcFqdn' failed: $($_.Exception.Message)" 'WARN'
        }
    }

    $serverParams = @{}
    if ($script:VCenterServer) { $serverParams['Server'] = $script:VCenterServer }

    $rows = @()
    $cluster = $null
    try { $cluster = Get-Cluster @serverParams -Name $clusterName -ErrorAction Stop }
    catch { Write-Log "vCenter cluster lookup by name '$clusterName' failed: $($_.Exception.Message)" 'WARN' }

    if ($cluster) {
        $vmhosts = @(Get-VMHost @serverParams -Location $cluster -ErrorAction Stop)
        foreach ($h in $vmhosts) {
            $hv = Get-View @serverParams -Id $h.Id -ErrorAction Stop
    ...
    if ($rows.Count -eq 0 -and $sddcClusterMor) {
        $viewIds = @($sddcClusterMor)
        if ($sddcClusterMor -notmatch '^ClusterComputeResource-') { $viewIds += ("ClusterComputeResource-{0}" -f $sddcClusterMor) }
        foreach ($vid in ($viewIds | Select-Object -Unique)) {
            try {
                $cv = Get-View @serverParams -Id $vid -ErrorAction Stop
                Write-Log "vCenter fallback using cluster MoRef '$vid' ($($cv.Name))." 'INFO'
                foreach ($hostRef in @($cv.Host)) {
                    $hv = Get-View @serverParams -Id $hostRef -ErrorAction Stop
                    foreach ($ps in @($hv.Config.Network.ProxySwitch)) {
                        $vds = ([string]$ps.DvsName).Trim()
                        if ($vds) { $rows += [pscustomobject]@{ vmhost=$hv.Name; vdsName=$vds; pnicDevices=@($ps.Pnic) } }
                    }
                }
                if ($rows.Count -gt 0) { break }
            } catch { Write-Log "vCenter cluster lookup by MoRef '$vid' failed: $($_.Exception.Message)" 'WARN' }
        }
    }

    if ($rows.Count -eq 0) {
        $currentVc = ''
        try { if ($script:VCenterServer) { $currentVc = ([string]$script:VCenterServer.Name).Trim() } } catch {}
        throw "Unable to detect vDS from vCenter. SDDC cluster name='$clusterName', SDDC vCenter='$sddcVcFqdn', connected/current vCenter='$currentVc', cluster MoRef='$sddcClusterMor'. Verify vCenter FQDN for the selected cluster and rerun Detect Network."
    }
...
function Update-DetectedNetworkSummary {
    ...
    try {
        ...
        if ($script:cmbNsxVdsOverride) {
            $script:cmbNsxVdsOverride.Items.Clear()
            foreach ($v in @($DetectedNetwork.VdsNames | Where-Object { $_ })) { [void]$script:cmbNsxVdsOverride.Items.Add([string]$v) }
            if ($DetectedNetwork.PrimaryVdsName -and $script:cmbNsxVdsOverride.Items.Contains($DetectedNetwork.PrimaryVdsName)) { $script:cmbNsxVdsOverride.SelectedItem = $DetectedNetwork.PrimaryVdsName }
            elseif ($script:cmbNsxVdsOverride.Items.Count -gt 0) { $script:cmbNsxVdsOverride.SelectedIndex = 0 }
            $script:cmbNsxVdsOverride.IsEnabled = [bool]$script:chkOverrideNsxVds.IsChecked
        }
    } catch {}
}

function Clear-DetectedNetworkSummary {
    try {
        $script:lblDetectedVdsCount.Text = '0'
        $script:lblDetectedVdsNames.Text = 'Not detected'
        $script:lblDetectedUplinks.Text = 'Not detected'
        $script:txtVdsToNsxUplinkMap.Text = ''
        $script:txtActiveUplinks.Text = ''
        $script:txtStandbyUplinks.Text = ''
        if ($script:cmbNsxVdsOverride) { $script:cmbNsxVdsOverride.Items.Clear(); $script:cmbNsxVdsOverride.IsEnabled = $false }
    } catch {}
}
...
function Ensure-DefaultNetworkMappings {
    ...
    $vdsAll = @(Find-FlatValue -FlatPairs $flat -PathRegex @('\.vdsName$','\.dvsName$','\.distributedSwitchName$') -Mode All) | ForEach-Object { ([string]$_).Trim() } | Where-Object { $_ -and $_ -notmatch '^[vV][dD][sS]0?\d$' } | Select-Object -Unique
    $vds = @($vdsAll | Sort-Object @{ Expression = { if ($_ -match '(?i)(^|[-_])VDS0?1$|VDS0?1\b|01$') { 0 } else { 1 } } }, @{ Expression = { $_ } })
    ...
    if ($det.VdsCount -gt 1) { Write-Log "Multiple vDS detected; using NSX/overlay vDS '$($det.PrimaryVdsName)' for the NSX host-switch config. Host vmnic mapping will still include all detected vDS objects." 'INFO' }
...
function Convert-DetectedVmnicMapToText {
    param($DetectedNetwork)
    if (-not $DetectedNetwork -or -not $DetectedNetwork.VdsNames -or $DetectedNetwork.VdsNames.Count -lt 1) { throw 'Missing detected vDS names.' }
    $parts = New-Object System.Collections.Generic.List[string]
    $vmnic = 0
    foreach ($vds in @($DetectedNetwork.VdsNames | Where-Object { $_ })) {
        $parts.Add(("vmnic{0}->{1}/uplink1" -f $vmnic, $vds)) | Out-Null
        $vmnic++
        $parts.Add(("vmnic{0}->{1}/uplink2" -f $vmnic, $vds)) | Out-Null
        $vmnic++
    }
    return ($parts -join '; ')
}
function New-VmnicMapForClusterVds { param($DetectedNetwork) return (Convert-DetectedVmnicMapToText -DetectedNetwork $DetectedNetwork) }
...
function Get-SelectedNsxVdsName {
    param($DetectedNetwork)
    $auto = ''
    if ($DetectedNetwork -and $DetectedNetwork.PrimaryVdsName) { $auto = ([string]$DetectedNetwork.PrimaryVdsName).Trim() }
    elseif ($DetectedNetwork -and $DetectedNetwork.VdsNames -and $DetectedNetwork.VdsNames.Count -gt 0) { $auto = ([string]$DetectedNetwork.VdsNames[0]).Trim() }
    if ($script:chkOverrideNsxVds -and $script:chkOverrideNsxVds.IsChecked) {
        $manual = ''
        if ($script:cmbNsxVdsOverride -and $script:cmbNsxVdsOverride.SelectedItem) { $manual = ([string]$script:cmbNsxVdsOverride.SelectedItem).Trim() }
        if ([string]::IsNullOrWhiteSpace($manual)) { throw 'NSX vDS override is enabled, but no vDS was selected.' }
        Write-Log "NSX vDS override enabled; using '$manual' for the NSX host-switch config. AZ2 host vmnic mapping will still include all detected cluster vDS objects." 'INFO'
        return $manual
    }
    if ([string]::IsNullOrWhiteSpace($auto)) { throw 'Unable to determine NSX vDS name.' }
    return $auto
}
...
function Test-WitnessReadiness {
    param([string]$WitnessFqdn)
    if ([string]::IsNullOrWhiteSpace($WitnessFqdn)) { throw 'Witness FQDN is required.' }
    $fqdn = $WitnessFqdn.Trim()
    try {
        $dns = Resolve-DnsName -Name $fqdn -ErrorAction Stop | Where-Object { $_.IPAddress -or $_.NameHost } | Select-Object -First 1
        if (-not $dns) { throw 'No DNS A/AAAA/CNAME response returned.' }
        Write-Log "Witness DNS resolution passed: $fqdn" 'INFO'
    } catch {
        throw "Witness FQDN DNS resolution failed for '$fqdn'. Correct the Witness FQDN before continuing. Detail: $($_.Exception.Message)"
    }
    if (-not $script:VCenterVerified -or -not $script:VCenterServer) {
        throw 'Witness vCenter inventory check requires a verified vCenter session. Click Verify vCenter and try again.'
    }
    try {
        Import-Module VMware.VimAutomation.Core -ErrorAction SilentlyContinue | Out-Null
        $vmh = Get-VMHost -Server $script:VCenterServer -Name $fqdn -ErrorAction SilentlyContinue | Select-Object -First 1
        if (-not $vmh) {
            $short = ($fqdn -split '\.')[0]
            $vmh = Get-VMHost -Server $script:VCenterServer -Name $short -ErrorAction SilentlyContinue | Select-Object -First 1
        }
        if (-not $vmh) { throw "Host not found in vCenter inventory as '$fqdn'." }
        Write-Log "Witness vCenter inventory check passed: $fqdn" 'INFO'
    } catch {
        throw "Witness vCenter inventory check failed for '$fqdn'. Add or correct the Witness host in vCenter inventory before continuing. Detail: $($_.Exception.Message)"
    }
}
...
function Build-SpecFromUi {
    ...
    $nsxVdsName = Get-SelectedNsxVdsName -DetectedNetwork $det
    Write-Log ("Host vmnic mapping will include all detected cluster vDS objects for SDDC validation: {0}. NSX host-switch config will use: {1}." -f (($det.VdsNames | Where-Object { $_ }) -join ', '), $nsxVdsName) 'INFO'
...
function Get-UnassignedUseableHosts {
    ...
    foreach ($h in $hosts) {
        ...
        $isAssigned = -not [string]::IsNullOrWhiteSpace(([string]$clusterId))
        $looksUnassigned = ($stateText -match 'UNASSIGNED') -or (-not $isAssigned)
        $looksUsable = ($stateText -match 'USEABLE|USABLE|READY|ACTIVE')

        # SDDC Manager commonly reports unassigned usable hosts as UNASSIGNED_USEABLE.
        # If clusterId is populated, never offer the host as an AZ2 candidate.
        if (-not $isAssigned -and $looksUnassigned -and ($looksUsable -or $stateText -match 'UNASSIGNED')) {
            $rows += [pscustomobject]@{
                FQDN = ([string]$fqdn)
                Id = ([string]$hostId)
                Status = ([string]$h.status)
                State = ([string]$h.state)
                Assignment = ([string]$h.assignmentState)
                Details = $stateText
            }
        }
    }

    return @($rows | Sort-Object FQDN)
}

function Show-Az2HostPicker {
    param([psobject]$Session)

    $data = @(Get-UnassignedUseableHosts -Session $Session)
    if ($data.Count -eq 0) {
        [System.Windows.MessageBox]::Show(
            'No UNASSIGNED/USEABLE hosts were found in SDDC Manager. Verify the AZ2 hosts are commissioned and not already assigned to a cluster.',
            'Select AZ2 Hosts',
            'OK',
            'Warning'
        ) | Out-Null
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
  ...
  <Grid Margin="10">
    ...
    <TextBlock Grid.Row="0" Text="Select only the UNASSIGNED/USEABLE hosts that should be used for AZ2." Margin="0,0,0,8"/>
    <DataGrid Grid.Row="1" Name="dgHosts" AutoGenerateColumns="False" SelectionMode="Extended" SelectionUnit="FullRow" CanUserAddRows="False" IsReadOnly="True">
      <DataGrid.Columns>
        <DataGridTextColumn Header="FQDN" Binding="{Binding FQDN}" Width="300"/>
        <DataGridTextColumn Header="Host ID" Binding="{Binding Id}" Width="260"/>
    ...
    <StackPanel Grid.Row="2" Orientation="Horizontal" HorizontalAlignment="Right" Margin="0,10,0,0">
      <Button Name="btnOk" Content="Use Selected Hosts" Width="140" Margin="4"/>
      <Button Name="btnCancel" Content="Cancel" Width="90" Margin="4"/>
    </StackPanel>
  </Grid>
</Window>
"@

    $picker = [Windows.Markup.XamlReader]::Parse($xamlHostPicker)
    try { if ($script:window) { $picker.Owner = $script:window } } catch {}
    $dg = $picker.FindName('dgHosts')
    $dg.ItemsSource = $data

    $picker.FindName('btnCancel').Add_Click({
        $picker.Tag = $null
        $picker.DialogResult = $false
        $picker.Close()
    })

    $picker.FindName('btnOk').Add_Click({
        $selected = @($dg.SelectedItems | ForEach-Object { $_.FQDN } | Where-Object { $_ })
        if ($selected.Count -eq 0) {
            [System.Windows.MessageBox]::Show('Select one or more AZ2 hosts first.', 'Select AZ2 Hosts', 'OK', 'Warning') | Out-Null
            return
        }
        $picker.Tag = $selected
    ...
    if ($picker.Tag) { return @($picker.Tag) }
    return @()
}
# =============================================================================
# UI
...
$xaml = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml" Title="VCF 9 Stretch Cluster Automation v$Global:VCFStretchVersion" Height="980" Width="1600" Background="#0C0C0C" Foreground="#E6E6E6" FontFamily="Segoe UI" FontSize="12" WindowStartupLocation="CenterScreen">
<Window.Resources>
<Style TargetType="GroupBox"><Setter Property="Foreground" Value="#E6E6E6"/><Setter Property="Margin" Value="8"/><Setter Property="Padding" Value="6"/><Setter Property="BorderBrush" Value="#BDBDBD"/></Style>
<Style TargetType="TextBlock"><Setter Property="Foreground" Value="#E6E6E6"/><Setter Property="VerticalAlignment" Value="Center"/></Style>
<Style TargetType="TextBox"><Setter Property="Background" Value="#202020"/><Setter Property="Foreground" Value="#E6E6E6"/><Setter Property="BorderBrush" Value="#6E6E6E"/><Setter Property="Margin" Value="4"/></Style>
<Style TargetType="PasswordBox"><Setter Property="Background" Value="#202020"/><Setter Property="Foreground" Value="#E6E6E6"/><Setter Property="BorderBrush" Value="#6E6E6E"/><Setter Property="Margin" Value="4"/></Style>
<Style TargetType="Button"><Setter Property="Margin" Value="4"/><Setter Property="Padding" Value="8,3"/><Setter Property="Background" Value="#2B2B2B"/><Setter Property="Foreground" Value="#E6E6E6"/><Setter Property="BorderBrush" Value="#BDBDBD"/></Style>
<Style TargetType="CheckBox"><Setter Property="Foreground" Value="#E6E6E6"/></Style>
</Window.Resources>
<Grid Margin="10">
<Grid.RowDefinitions><RowDefinition Height="0"/><RowDefinition Height="Auto"/><RowDefinition Height="Auto"/><RowDefinition Height="Auto"/><RowDefinition Height="Auto"/><RowDefinition Height="Auto"/><RowDefinition Height="*"/><RowDefinition Height="Auto"/></Grid.RowDefinitions>
<GroupBox Grid.Row="1" Header="Prerequisites"><Grid><Grid.ColumnDefinitions><ColumnDefinition Width="200"/><ColumnDefinition Width="200"/><ColumnDefinition Width="240"/><ColumnDefinition Width="240"/><ColumnDefinition Width="240"/><ColumnDefinition Width="Auto"/><ColumnDefinition Width="Auto"/><ColumnDefinition Width="Auto"/><ColumnDefinition Width="Auto"/></Grid.ColumnDefinitions><TextBlock Name="lblPS" Grid.Column="0"/><TextBlock Name="lblWPF" Grid.Column="1"/><TextBlock Name="lblImpExcel" Grid.Column="2"/><TextBlock Name="lblPCLI" Grid.Column="3"/><TextBlock Name="lblVCFPCLI" Grid.Column="4"/><Button Name="btnRecheck" Grid.Column="5" Content="Recheck"/><Button Name="btnInstallImportExcel" Grid.Column="6" Content="Install ImportExcel"/><Button Name="btnInstallPCLI" Grid.Column="7" Content="Install PowerCLI"/><Button Name="btnInstallVCFPCLI" Grid.Column="8" Content="Install VCF.PowerCLI"/></Grid></GroupBox>
<GroupBox Grid.Row="2" Header="Connections"><Grid><Grid.ColumnDefinitions><ColumnDefinition Width="120"/><ColumnDefinition Width="310"/><ColumnDefinition Width="100"/><ColumnDefinition Width="270"/><ColumnDefinition Width="115"/><ColumnDefinition Width="170"/><ColumnDefinition Width="125"/><ColumnDefinition Width="*"/></Grid.ColumnDefinitions><Grid.RowDefinitions><RowDefinition Height="Auto"/><RowDefinition Height="Auto"/><RowDefinition Height="Auto"/></Grid.RowDefinitions><TextBlock Grid.Row="0" Grid.Column="0" Text="SDDC Manager:"/><TextBox Grid.Row="0" Grid.Column="1" Name="txtSddcHost"/><TextBlock Grid.Row="0" Grid.Column="2" Text="User:"/><TextBox Grid.Row="0" Grid.Column="3" Name="txtSddcUser" Text="administrator@vsphere.local"/><TextBlock Grid.Row="0" Grid.Column="4" Text="Password:"/><PasswordBox Grid.Row="0" Grid.Column="5" Name="pbSddcPass"/><Button Grid.Row="0" Grid.Column="6" Name="btnConnect" Content="Connect SDDC"/><TextBlock Grid.Row="0" Grid.Column="7" Name="lblConnStatus" Text="Not connected" Foreground="#D6C15A"/><TextBlock Grid.Row="1" Grid.Column="0" Text="vCenter FQDN:"/><TextBox Grid.Row="1" Grid.Column="1" Name="txtVCenterFqdn"/><TextBlock Grid.Row="1" Grid.Column="2" Text="User:"/><TextBox Grid.Row="1" Grid.Column="3" Name="txtVCenterUser" Text="administrator@vsphere.local"/><TextBlock Grid.Row="1" Grid.Column="4" Text="Password:"/><PasswordBox Grid.Row="1" Grid.Column="5" Name="pbVCenterPass"/><Button Grid.Row="1" Grid.Column="6" Name="btnVerifyVCenter" Content="Verify vCenter"/><TextBlock Grid.Row="1" Grid.Column="7" Name="lblVCenterStatus" Text="Not verified" Foreground="#D6C15A"/><TextBlock Grid.Row="2" Grid.Column="0" Text="NSX Manager:"/><TextBox Grid.Row="2" Grid.Column="1" Name="txtNsxHost"/><TextBlock Grid.Row="2" Grid.Column="2" Text="User:"/><TextBox Grid.Row="2" Grid.Column="3" Name="txtNsxUser" Text="admin"/><TextBlock Grid.Row="2" Grid.Column="4" Text="Password:"/><PasswordBox Grid.Row="2" Grid.Column="5" Name="pbNsxPass"/><Button Grid.Row="2" Grid.Column="6" Name="btnConnectNsx" Content="Connect NSX"/><TextBlock Grid.Row="2" Grid.Column="7" Name="lblNsxStatus" Text="NSX not connected" Foreground="#D6C15A"/></Grid></GroupBox>
...
function Refresh-TopologyUI { $hasSession=$script:SddcSession -ne $null; $hasCluster=$script:cmbCluster.SelectedItem -ne $null; $hasDetected=$script:DetectedNetwork -ne $null; $vcOk=(-not $script:chkRequireVCenterVerify.IsChecked) -or $script:VCenterVerified; $script:btnPickHosts.IsEnabled=$hasSession; $script:btnDetectNetwork.IsEnabled=$hasSession -and $hasCluster; if($script:cmbNsxVdsOverride){$script:cmbNsxVdsOverride.IsEnabled=$hasDetected -and [bool]$script:chkOverrideNsxVds.IsChecked}; $can=$hasSession -and $hasCluster -and $hasDetected -and $vcOk; $script:btnGenerate.IsEnabled=$can; $script:btnValidate.IsEnabled=$can; $script:btnExecute.IsEnabled=$can }
...
# Events
$script:window.Add_ContentRendered({ if(-not $script:RunDir){New-RunDir -Base $script:ReportsBase | Out-Null}; $script:txtReportsPath.Text=$script:ReportsBase; Select-ComboByContent $script:cmbTeamingPolicy 'LOADBALANCE_SRCID'; Prereq-Check | Out-Null; Write-Log "==== VCF Stretch UI started v$Global:VCFStretchVersion ===="; Write-Log "Run folder: $script:RunDir"; Write-Log 'Prerequisite installs run in background to keep the UI responsive.'; Set-RunStatus -Text "Ready" -State "Ready"; Refresh-TopologyUI })
$script:btnRecheck.Add_Click({ Prereq-Check | Out-Null })
$script:btnInstallImportExcel.Add_Click({ Install-RequiredModule -Name 'ImportExcel' -FriendlyName 'ImportExcel' })
$script:btnInstallPCLI.Add_Click({ Install-RequiredModule -Name 'VMware.PowerCLI' -FriendlyName 'VMware.PowerCLI' -AcceptLicense })
$script:btnInstallVCFPCLI.Add_Click({ Install-RequiredModule -Name 'VCF.PowerCLI' -FriendlyName 'VCF.PowerCLI' -AcceptLicense })
$script:chkOverrideNsxVds.Add_Click({ Refresh-TopologyUI; if($script:chkOverrideNsxVds.IsChecked -and $script:cmbNsxVdsOverride.SelectedItem){ Write-Log "NSX vDS override selected: $($script:cmbNsxVdsOverride.SelectedItem)" 'INFO' } })
$script:cmbNsxVdsOverride.Add_SelectionChanged({ if($script:chkOverrideNsxVds.IsChecked -and $script:cmbNsxVdsOverride.SelectedItem){ Write-Log "NSX vDS override changed to: $($script:cmbNsxVdsOverride.SelectedItem)" 'INFO' } })
...
$script:btnOpenOut.Add_Click({
    try {
        if (-not $script:RunDir -or -not (Test-Path -LiteralPath $script:RunDir)) { throw 'No valid run folder exists yet.' }
        Start-Process -FilePath 'explorer.exe' -ArgumentList "`"$script:RunDir`""
        Write-Log "Opened output folder: $script:RunDir"
    } catch {
        Write-Log "Open Output failed: $($_.Exception.Message)" 'ERROR'
        Show-ErrorMessage -Message 'Open Report failed. Check the log for details.' -Title 'Open Report'
    }
})
...
$script:btnPickHosts.Add_Click({
    try {
        if (-not $script:SddcSession) { throw 'Connect to SDDC Manager first.' }
        $selectedHosts = @(Show-Az2HostPicker -Session $script:SddcSession)
        if ($selectedHosts.Count -gt 0) {
            $script:txtAz2Hosts.Text = ($selectedHosts -join "`r`n")
            Write-Log "Selected AZ2 hosts: $($selectedHosts -join ', ')"
        } else {
            Write-Log 'AZ2 host picker closed without changing host selection.' 'INFO'
        }
    } catch {
        Write-Log "Host picker failed: $($_.Exception.Message)" 'ERROR'
        [System.Windows.MessageBox]::Show($_.Exception.Message, 'Host picker failed', 'OK', 'Error') | Out-Null
    }
})
$script:btnGenerate.Add_Click({ try { Set-Busy $true; $spec=Build-SpecFromUi; $ts=Get-Date -Format yyyyMMdd-HHmmss; $cid=Get-SelectedClusterId; $out=Join-Path $script:RunDir "clusterStretchSpec_$cid`_$ts.json"; $wrap=Join-Path $script:RunDir "clusterUpdateSpec_validationWrapper_$cid`_$ts.json"; $spec|ConvertTo-Json -Depth 90|Set-Content $out -Encoding UTF8; @{clusterUpdateSpec=$spec}|ConvertTo-Json -Depth 100|Set-Content $wrap -Encoding UTF8; Write-Log "Wrote JSON: $out"; Write-Log "Wrote validation wrapper: $wrap"; Show-InfoMessage -Message 'Successfully Generated Specification' -Title 'Generate JSON'; Set-Busy $false } catch { Write-Log "Generate failed: $($_.Exception.Message)" ERROR; Set-RunStatus -Text "Failed" -State "Failed"; Show-ErrorMessage -Message "Generate JSON failed. Check the log for details." -Title "Generate JSON"; Set-Busy $false } })
$script:btnValidate.Add_Click({
    try {
        Set-Busy $true
        $spec = Build-SpecFromUi
        $cid = Get-SelectedClusterId
        $session = $script:SddcSession
        $runDir = $script:RunDir
        Start-BackgroundTask -TaskName 'Validate' -SuccessTitle 'Validate' -SuccessMessage 'Validation Success' -ArgumentList @($session,$cid,$spec,$runDir) -ScriptBlock {
            param($session,$cid,$spec,$runDir)
            function Invoke-LocalSddcApi {
                param($Session,[string]$Method,[string]$Path,[object]$Body=$null)
                $uri = "$($Session.Base)$Path"
                $headers = @{ Authorization = "Bearer $($Session.AccessToken)"; Accept='application/json' }
                if ($null -ne $Body) {
            ...
            Write-Output "Validating stretch spec via POST /v1/clusters/$cid/validations..."
            $res = Invoke-LocalSddcApi -Session $session -Method POST -Path ("/v1/clusters/{0}/validations" -f $cid) -Body @{clusterUpdateSpec=$spec}
            $out = Join-Path $runDir ("ValidationResponse_{0}_{1}.json" -f $cid,(Get-Date -Format yyyyMMdd-HHmmss))
            $res | ConvertTo-Json -Depth 90 | Set-Content $out -Encoding UTF8
            Write-Output "Validation response saved: $out"
        }
    } catch {
        Write-Log "Validate failed: $($_.Exception.Message)" 'ERROR'
        Set-RunStatus -Text 'Failed' -State 'Failed'
        Show-ErrorMessage -Message 'Validation Failed. Check the log for details.' -Title 'Validate'
        Set-Busy $false
    }
})
$script:btnExecute.Add_Click({
    try {
        ...
        $spec = Build-SpecFromUi
        $cid = Get-SelectedClusterId
        $ans = [System.Windows.MessageBox]::Show("This will PATCH /v1/clusters/$cid and start stretch. Continue?",'Execute','YesNo','Warning')
        if ($ans -ne 'Yes') { Set-Busy $false; return }
        $session = $script:SddcSession
        $runDir = $script:RunDir
        Start-BackgroundTask -TaskName 'Execute' -SuccessTitle 'Execute' -SuccessMessage 'Task started. Monitor progress in VCF Operations Tasks.' -ArgumentList @($session,$cid,$spec,$runDir) -ScriptBlock {
            param($session,$cid,$spec,$runDir)
            function Invoke-LocalSddcApi {
                param($Session,[string]$Method,[string]$Path,[object]$Body=$null)
                $uri = "$($Session.Base)$Path"
            ...
            Write-Output "Submitting stretch task via PATCH /v1/clusters/$cid..."
            $res = Invoke-LocalSddcApi -Session $session -Method PATCH -Path ("/v1/clusters/{0}" -f $cid) -Body $spec
            $out = Join-Path $runDir ("ExecuteResponse_{0}_{1}.json" -f $cid,(Get-Date -Format yyyyMMdd-HHmmss))
            $res | ConvertTo-Json -Depth 90 | Set-Content $out -Encoding UTF8
            Write-Output "Execute response saved: $out"
        }
    } catch {
        Write-Log "Execute failed: $($_.Exception.Message)" 'ERROR'
        Set-RunStatus -Text 'Failed' -State 'Failed'
        Show-ErrorMessage -Message 'Execute Failed. Check the log for details.' -Title 'Execute'
        Set-Busy $false
    }
})
